...
void resistance_to_str(uint32_t resistance, char *str) {
    ...
    if (resistance < 1000) {
        // Display as ohms for values less than 1k
        sprintf(str, "%4d Ohm", resistance);
    } else {
        // Display as kilo-ohms for values 1k and above
        sprintf(str, "%3d.%1d kOhm", resistance/1000, (resistance%1000)/100);
    }
}
        ...
        if (measurement_active) {
            ...
            while (min_index <= max_index) {
                ...
                if ((GPIOA->DATA & 0x80) == 0) {
                    // Voltage is below 2V, need higher current (lower index)
                    max_index = mid_index - 1;
                } else {
                    // Voltage is at or above 2V, need lower current (higher index)
